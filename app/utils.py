...
# Разрешённые форматы загружаемых изображений
...
async def validate_upload_image(file: UploadFile) -> Optional[str]:
    """
    Проверяет загружаемый файл изображения на допустимый формат и размер.

    Валидирует расширение, MIME-тип и размер файла. Если файл корректен,
    указатель чтения возвращается в начало, чтобы содержимое можно было
    прочитать повторно (например, для загрузки в S3).

    Аргументы:
        file (UploadFile): Загружаемый файл изображения.

    Возвращает:
        Optional[str]: Текст ошибки или None, если файл прошёл проверку.
    """
    ...
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        return "Неподдерживаемый формат изображения. Разрешены: PNG, JPG, GIF, WEBP"

    if file.content_type and file.content_type not in ALLOWED_IMAGE_CONTENT_TYPES:
        return "Неподдерживаемый MIME-тип файла"
    ...
    if len(content) > max_bytes:
        return f"Файл слишком большой! Максимум {settings.MAX_UPLOAD_SIZE_MB} МБ."
...
async def upload_image_to_s3(file: UploadFile, prefix: str = "maps") -> str:
    """
    Загружает изображение в S3/MinIO и возвращает относительный URL.

    Перед загрузкой выполняет валидацию расширения, MIME-типа и размера файла.

    Аргументы:
        file (UploadFile): Загружаемый файл изображения.
        prefix (str): Префикс ключа объекта в бакете (например, "maps").

    Возвращает:
        str: Относительный URL файла вида "/media/{prefix}/{имя}".

    Исключения:
        HTTPException: если файл не прошёл валидацию (статус 400).
    """
    error = await validate_upload_image(file)
    if error:
...
def get_random_quote():
    ...
    quotes_file = settings.DATA_DIR / "quotes.json"
