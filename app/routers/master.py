...
from app.models import RewardRequest, RollRequest
from app.dependencies import get_current_user, templates, get_room_service, get_combat_service, get_file_service, get_character_service, require_room_master
...
async def api_upload_scene(
    ...
    try:
        ...
        scene = room_service.create_scene(room_id, name, background_url, width, height)
        ...
        # Ошибки валидации файла пробрасываем как есть (статус 400)
        raise
    except Exception as e:
...
async def api_get_scenes(room_id: int, current_user: dict = Depends(get_current_user)):
    ...
    scenes = room_service.get_room_scenes(room_id)
    for s in scenes:
...
async def api_activate_scene(room_id: int, scene_id: int, current_user: dict = Depends(get_current_user)):
    ...
    room_service.set_active_scene(room_id, scene_id)
...
async def api_delete_scene(room_id: int, scene_id: int, room: dict = Depends(require_room_master)):
    scene = room_service.delete_scene(room_id, scene_id)
    if not scene:
        return JSONResponse(status_code=404, content={"status": "error", "message": "Сцена не ��айдена"})
...
async def api_upload_prop(
    ...
    try:
        ...
        image_url = await file_service.upload_image_to_s3(file, prefix="props")
