...
from app.models import TokenUpload
from app.dependencies import require_player, get_current_user, check_character_ownership, templates, get_character_service
from app.services.character_service import CharacterService

from app.schemas.constants import (
    DND_RACES, DND_CLASSES, DND_ALIGNMENTS,
    DND_STATS, DND_STATS_RU, DND_SKILLS, DND_SAVES, XP_THRESHOLDS,
)

from app.utils import (
    CLASSES_DATA, EQUIPMENT_DATA,
    get_all_spells, get_random_quote, load_equipment,
    upload_image_to_s3, validate_upload_image
)
...
async def create_character(request: Request, current_user: dict = Depends(require_player)):
    ...
    try:
        ...
        level = character_service.calc_level_from_xp(xp)
        ...
        for stat_key in DND_STATS:
            ...
            stats[stat_key] = {
                'score': score,
                'modifier': character_service.calc_modifier(score)
            }
        ...
        character_service.save_chars(current_user['username'], chars)
...
async def view_char(char_id: int, request: Request, current_user: dict = Depends(require_player)):
    ...
    char = character_service.normalize_char(char)
    char['_calculated_ac'] = character_service.calculate_ac(char)

    saves, skills = character_service.prepare_skills_and_saves(char)
    features = character_service.get_class_features(char.get('char_class', ''), char.get('subclass', 'Нет'), char.get('level', 1))
...
async def save_char_edit(char_id: int, request: Request, current_user: dict = Depends(require_player)):
    ...
    try:
        ...
        char['attributes']['prof_bonus'] = f"+{character_service.calc_prof_bonus(char['level'])}"
...
async def update_hp(
    ...
    user_chars = character_service.load_chars(username)
    ...
    character_service.save_chars(username, user_chars)
...
async def upload_token(char_id: int, token_data: TokenUpload, current_user: dict = Depends(require_player)):
    ...
    if token_data.image.startswith('data:image'):
        try:
            ...
            # Проверка размера изображения токена
            max_bytes = utils.settings.MAX_UPLOAD_SIZE_MB * 1024 * 1024
            if len(image_bytes) > max_bytes:
                return {"status": "error",
                        "error": f"Файл слишком большой! Максимум {utils.settings.MAX_UPLOAD_SIZE_MB} МБ."}
...
async def save_description(
    ...
    try:
        ...
        if remove_image == "1":
            ...
            # Валидация формата и размера файла (общая для всех загрузок изображений)
            upload_error = await validate_upload_image(char_image)
            if upload_error:
                return templates.TemplateResponse(request, "char_description.html", context={
                    "char": char,
                    "error": upload_error,
...
async def buy_equipment(char_id: int, request: Request, current_user: dict = Depends(require_player)):
    ...
    try:
        if eq_type == 'weapon':
            ...
            if found_item:
                dmg = character_service.parse_damage(found_item.get('damage', '1d4'))
                w_type = character_service.map_weapon_type(found_item.get('properties', []))
                char_class = char.get('char_class', '')
                is_proficient = character_service.determine_weapon_proficiency(char_class, weapon_category, item_name)
...
async def view_class_features(char_id: int, request: Request, current_user: dict = Depends(require_player)):
    ...
    char = character_service.normalize_char(char)
    features = character_service.get_class_features(
        char.get('char_class', ''),
